...
class Comment_user_page():
    ...
    def read_page(self, driver, user):
        ...
        Files().save_to_target_list(user, self.head_info[1], self.head_info[2], self.head_info[3],
                                    self.head_info[4])
    ...
    def check_if_user_right(self, driver):
        ...
        if word_count(counts_of_following) == 1:
            if 5 < int(counts_of_following) < 500:
                ...
                print('Пользователь не подходит')
                self.head_info[4] = "########################"
        else:
            print('Пользователь не подходит')
            self.head_info[4] = "########################"
